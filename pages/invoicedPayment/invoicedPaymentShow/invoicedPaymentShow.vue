<template>
  <view>
    <view>
      <u-card
        full
        :title="model.invoicedDate"
        :sub-title="model.invoice_no"
        :margin="card.margin"
    ...
    <permit-checker authKey="invoiced_payment#show" ></permit-checker>
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    async onLoad(options) {
      ...
      let model = await this.fetchInvoicedPaymentShow({ id });

      this.id = id;
      this.editUrl = `/pages/invoicedPayment/invoicedPaymentEdit/invoicedPaymentEdit?id=${id}`;
    ...
    methods: {
      ...
      async fetchInvoicedPaymentShow ({ id }) {
        uni.showLoading({
          title: '加载中'
        });

        let res = await invoicedPaymentApi.show({id});
        let {
          data: {
