<template>
  <view class="wrap">
    <custom-form
      ...
      v-if="formReady"
    />
    <permit-checker authKey="invoiced_payment#edit" ></permit-checker>
  </view>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      handleSave(values) {
        ...
        invoicedPaymentApi.update({id, invoiced_payment: values}).then((res)=> {
          let { data: {code, remark} } = res;

          if (Number(code) === 0) {
              uni.navigateTo({
                url: `/pages/invoicedPayment/invoicedPaymentShow/invoicedPaymentShow?id=${id}`,
                success() {
                  _.delay(()=>{
                    uni.showToast({
                      icon: 'success',
