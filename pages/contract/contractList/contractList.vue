      ...
      >
        <uni-list-item>
          <u-card
            ...
            :border="card.border" :sub-title="item.sn" :full="card.full"
  ...
  export default {
    ...
    async onLoad() {
      ...
      this.ShowCustomFields = _(customFields).filter((customField) => {
        ...
        _.includes(["title", "sn", "status", "opportunity"], customField.name)
      ).value().slice(0, 5);

      uni.setNavigationBarTitle({
          title: this.featureLabels["contract"]
      });

      // 初始化页面数据
      this.fetchContract({reload: true });
    },
    methods: {
      /**
       * 下拉刷新回调函数
       ...
       */
      fetchContract({ reload = false, page = 1}) {
        this.status = 'loading';
        let {
          query,
          sort,
        ...
        contractApi.index({ page, per_page, query, search_column_name, filters, sort }).then((res) => {
          _.delay(()=>{
            uni.hideLoading();
          }, 100)
      ...
      handleItemClick (event, id) {
        let url = `/pages/contract/contractShow/contractShow?id=${id}`
        uni.navigateTo({
          url
        });
      },
