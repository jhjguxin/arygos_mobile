  ...
  <view class="list">
    ...
    <uni-list>
      ...
      >
        <uni-list-item>
          <u-card
            class="item-body" slot="body" :title="item.receive_stage_display"
          ...
          <u-row class="item-footer" slot="footer" gutter="0" align="center">
            <u-tag :text="item.status_display" v-if="item.status_display"/>
          </u-row>
        </uni-list-item>
      </u-swipe-action>
    </uni-list>
  ...
  export default {
    ...
    async onLoad() {
      ...
      this.ShowCustomFields = _(customFields).filter((customField) => {
        ...
        _.includes(["receive_date", "receive_stage", "status"], customField.name)
      ).value().slice(0, 5);

      uni.setNavigationBarTitle({
        title: featureLabels["received_payment_plan"]
      });

      // 初始化页面数据
      this.fetchReceivedPaymentPlan({reload: true });
    },
    methods: {
      /**
       * 下拉刷新回调函数
      ...
      onReachBottom() {
        ...
        if (status == 'more') {
          page += 1;
          this.page = page;
          this.fetchReceivedPaymentPlan({ page });
        }
      },
      /**
       * 获取页面数据
       ...
       */
      fetchReceivedPaymentPlan({ reload = false, page = 1}) {
        this.status = 'loading';
        let {
          query,
          sort,
        ...
        receivedPaymentPlanApi.index({ page, per_page, query, search_column_name, filters, sort }).then((res) => {
          _.delay(()=>{
            uni.hideLoading();
          }, 100)
      ...
      handleItemClick (event, id) {
        let url = `/pages/receivedPaymentPlan/receivedPaymentPlanShow/receivedPaymentPlanShow?id=${id}`
        uni.navigateTo({
          url
        });
      },
      ...
      handleSearch ({query, searchColumnName }) {
        ...
        this.fetchreceived_payment_plan({ reload: true });
      },
