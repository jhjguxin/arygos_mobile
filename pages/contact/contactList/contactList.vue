      ...
      >
        <uni-list-item>
          <u-card
            ...
            :border="card.border" :sub-title="item.extra.job" :full="card.full"
  ...
  export default {
    ...
    async onLoad() {
      ...
      this.ShowCustomFields = _(customFields).filter((customField) => {
        ...
        _.includes(["name", "extra.job"], customField.name)
      ).value().slice(0, 5);
    ...
    methods: {
       ...
       */
      fetchContact({ reload = false, page = 1}) {
        this.status = 'loading';
        let {
          query,
          sort,
        ...
        contactApi.index({ page, per_page, query, search_column_name, filters, sort }).then((res) => {
          _.delay(()=>{
            uni.hideLoading();
          }, 100)
      ...
      handleItemClick (event, id) {
        let url = `/pages/contact/contactShow/contactShow?id=${id}`
        uni.navigateTo({
          url
        });
      },
