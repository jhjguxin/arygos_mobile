  ...
  export default {
    ...
    async onLoad() {
      ...
      uni.setNavigationBarTitle({
        title: `${featureLabels['customer']}按用户排名`
      })
    ...
    methods: {
      ...
      fetchData(){
        ...
        reportCenterApi.customer_count(filters).then((res) => {
          let {
            data: {
              data
            },
          ...
          _.each(data.data.x, (_item, index) => {
            ...
            tableData[index] = {
              ...
              x: _item,
              key: index,
              count: data.data.y[index],
              department: data.data.department[index]
            }
          })
          this.tableHead = ["用户", "部门", "数量"];
          this.tableColumns = ["x", "department", "count"];
