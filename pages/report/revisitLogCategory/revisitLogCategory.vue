...
<script>
  ...
  export default {
    ...
    async onLoad() {
      ...
      let customFields = await CustomField.instance().fetchData("RevisitLog");
    ...
    methods: {
      ...
      fetchData(){
        ...
        reportCenterApi.revisit_log(filters).then((res) => {
          let {
            data: {
              data
            },
          ...
          _.each(data, (item) => {
            _.each(item.data.x, (_item, index) => {
              ...
              tableData[index] = {
                ...
                x: _item,
                key: index,
                [`${item.name}`]: item.data.y[index],
              }
            })
          });
          this.tableHead = _.concat(["用户"] , _.map(this.data, "label"));
          this.tableColumns = _.concat(["x"] , _.map(this.data, "name"));
