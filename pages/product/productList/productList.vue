<template>
  <view class="list">
    ...
    <uni-list>
      ...
      >
        <uni-list-item :thumb="item.thumb" thumbSize="lg">
          <u-card
            class="item-body" slot="body" :title="item.name"

            :border="card.border" :sub-title="item.no" :full="card.full"
  ...
  export default {
    ...
    async onLoad() {
      ...
      this.ShowCustomFields = _(customFields).filter((customField) => {
        ...
        _.includes(["name", "no", "status"], customField.name)
      ).value().slice(0, 5);
    ...
    methods: {
       ...
       */
      fetchProduct({ reload = false, page = 1}) {
        this.status = 'loading';
        let {
          query,
          sort,
        ...
        productApi.index({ page, per_page, query, search_column_name, filters, sort }).then((res) => {
          _.delay(()=>{
            uni.hideLoading();
          }, 100)
          ...
          const tempList = _.map(_models, (model) => (
            {
              ...
              thumb: model.image_attachments?.[0]?.file_url || "/static/img/thumb.png",
      ...
      handleItemClick (event, id) {
        let url = `/pages/product/productShow/productShow?id=${id}`
        uni.navigateTo({
          url
        });
      },
